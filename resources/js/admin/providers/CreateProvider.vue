<template>
    <div>
        ...
        <form v-if="!isLoading" @submit.stop.prevent="createProvider">
            <div class="row mb-3">
...
export default {
    ...
    methods: {
        createProvider() {
            this.isLoading = true;
            axios.post('/api/providers/create', this.provider)
                .then(() => {
                    this.isLoading = false;
                    this.$swal({
                        title: 'Добавлено!',
                        text: 'Поставщик успешно создан :)',
                        icon: 'success',
                    });
                    window.location.href = '/admin/bookkeeping/providers';
                })
                .catch(({response}) => {
